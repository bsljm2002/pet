...
class CustomBottomNav extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BottomNavigationBar(
      ...
      items: const [
        BottomNavigationBarItem(icon: Icon(Icons.home), label: '홈'), // 0번: 홈 (펫프로필/펫일기)
        BottomNavigationBarItem(icon: Icon(Icons.psychology), label: 'AI케어'), // 1번: AI케어 (케이지/AI진단)
        BottomNavigationBarItem(
          icon: Icon(Icons.medical_services),
          label: '동물병원', // 2번: 동물병원
        ),
        BottomNavigationBarItem(icon: Icon(Icons.settings), label: '설정'), // 3번: 설정
      ],
    );
  }
}
