...
class EditPetProfileScreen extends StatefulWidget {
  ...
  const EditPetProfileScreen({super.key, required this.profile});
...
class _EditPetProfileScreenState extends State<EditPetProfileScreen> {
  ...
  void initState() {
    ...
    _selectedBreed = widget.profile.speciesDetail;
  }
      ...
      child: ElevatedButton(
        onPressed: () async {
          ...
          try {
            ...
            final result = await PetService().updatePet(
            ...
            if (result['success'] == true) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('${_nameController.text.trim()} 프로필이 수정되었습니다!'),
                  backgroundColor: const Color.fromARGB(255, 0, 108, 82),
                ),
              );
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(result['message'] ?? '수정 실패'),
                  backgroundColor: Colors.red,
                ),
              );
            }
